...
const SendButton = ({ state, handleSubmit }: ButtonProps) => {
    const buttonState: 'pending' | 'success' | 'error' | 'off' = state.isPending ? 'pending' : state.isSuccess && !state.isError ? 'success' : !state.isSuccess && state.isError ? 'error' : 'off'


    return (
    <button className={`cursor-pointer flex items-center justify-center w-full py-3 px-5 rounded-[9px] ${buttonState == 'pending' ? 'bg-gray-400' : buttonState == 'error' ? 'bg-red-500 hover:bg-red-800' : buttonState == 'success' ? 'bg-green-400 hover:bg-green-700' : 'bg-blue-400 hover:bg-blue-700'} text-white transition-colors font-semibold relative border`} onClick={handleSubmit}>
		{buttonState == 'pending' ? (
              <div className='flex gap-3 items-center'>
                ...
                <p>Confirming in Wallet</p>
            </div>
		) : buttonState == 'error' ? <p>Error, try again</p> : buttonState == 'success' ? <p>Success</p> : <p>Send Tokens</p>}
	</button>
  )
}
